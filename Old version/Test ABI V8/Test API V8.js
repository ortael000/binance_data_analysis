...
strategy_button.addEventListener("click",async function(){
  ...
  title_table[0].push("Tendance");
  title_table[0].push("MACD vs EMA T-1");
  title_table[0].push("MACD vs EMA T");
  ...
  for (let i = 0; i < result_table.length; i++) {   // on rajoute le EMA1
    ...
    result_table[i].push(strat_table[i][3]);
    result_table[i].push(strat_table[i][4]);
    result_table[i].push(strat_table[i][5]);
  }
